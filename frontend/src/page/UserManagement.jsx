...
const UserManagement = () => {
  ...
  const fetchUsers = async () => {
    setLoading(true);
    try {
      const response = await fetch('http://18.232.139.209:5000/api/auth/users', {
        headers: { 
  ...
  const handleDeleteUser = async (userId) => {
    if (window.confirm(`⚠️ Nguy hiểm: Bạn có chắc chắn muốn XÓA VĨNH VIỄN người dùng này?`)) {
      try {
        const response = await fetch(`http://18.232.139.209:5000/api/auth/users/${userId}`, {
          method: 'DELETE',
          headers: { 
            'Authorization': `Bearer ${getToken()}`,
            'Content-Type': 'application/json' 
  ...
  const handleToggleStatus = async (user) => {
    ...
    if (window.confirm(`Bạn có chắc chắn muốn ${actionText} tài khoản của [${user.fullName}]?`)) {
      try {
        const response = await fetch(`http://18.232.139.209:5000/api/auth/users/${user.id}/status`, {
          method: 'PUT',
          headers: { 
            'Authorization': `Bearer ${getToken()}`,
            'Content-Type': 'application/json' 
        ...
        if (response.ok) {
          ...
          // Fallback
          const updateRes = await fetch(`http://18.232.139.209:5000/api/auth/users/${user.id}`, {
            method: 'PUT',
  ...
  const handleSaveUser = async () => {
    ...
    const url = isEditing ? `http://18.232.139.209:5000/api/auth/users/${formData.id}` : 'http://18.232.139.209:5000/api/auth/users';
