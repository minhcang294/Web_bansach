...
export default function ImportManagement() {
  ...
  const fetchImports = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/imports`);
      if (res.ok) {
  ...
  const handleSaveImport = async () => {
    ...
    const url = isEditing 
      ? `${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/imports/${formData.maPhieuNhap}` 
      : `${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/imports`;
  ...
  const handleDelete = async (maPhieuNhap) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/imports/${maPhieuNhap}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${getToken()}` }
      });
